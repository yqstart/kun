...
└── kun-app/                  # egui 应用
    ...
    ├── src/app.rs            # KunApp：布局/连接管理/对话框/快捷键/主题切换
    ├── src/theme.rs          # 四套主题 token + 终端调色板 + 主题切换
    └── src/views/
...
- 设计 token 在 `theme.rs::miro`：分层背景 `#0a0a0d`（应用）/`#141418`（标题栏）/`#1c1c22`（面板）/`#28282f`（浮层）
- 文字三阶 `#f5f5f7/#c7c7cc/#8e8e93`；accent `#8b5cf6` + `ACCENT_SOFT`（16% 透明）做 hover/选中底
- 边框统一 5% 半透明白；圆角 10px（按钮/输入）/6px（列表项）
- **四套主题**（`theme.rs::THEMES`）：Miro 深色 / Dawn 浅色 / Midnight 深蓝 / Cyberpunk 霓虹，每套含 UI token + 终端调色板
- 终端调色板（Catppuccin Mocha 16 色 + xterm 256 色表）在渲染层解析，优先级：Spec > OSC 覆盖 > 内置调色板；背景色强制跟随主题（忽略 OSC 11 背景覆盖，保证浅色主题一致性）
