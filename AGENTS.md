...
- **标签/状态栏标题跟随当前目录（2026-09-11）**：`TerminalTab::title()` 返回 `(显示文本, 全路径悬浮提示)`——**本地标签显示当前目录的末级文件夹名**（`dir_display_name`，zsh `%c` 语义：home → `~`、根 → `/`、尾随 `/` 剥掉），**悬浮标签标题/状态栏标题显示全路径**；远程标签仍用主机名（远端目录由 sshd 决定，本地跟踪器不适用）。目录来源是 `TerminalView::current_directory()`（终端输入跟踪，`cd` 回车后更新 + `pwd` 输出校正），**不采用 shell 上报的窗口标题**：oh-my-zsh 的标题是截断过的 `%15<..<%~%<<`，既非末级目录名也拿不到完整路径（曾固定显示 `label = "本地终端"`）。回归测试 `目录展示名只保留末级` / `本地标签标题跟随当前目录`（cd 进嵌套目录后标题 = 末级名、提示 = canonical 全路径）/ `标签栏与状态栏显示当前目录名`（完整应用渲染：标签栏 + 状态栏各一处，hover 标题出现全路径提示）
